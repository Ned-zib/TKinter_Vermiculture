...
class View():
    def __init__(self, root):
        ...
        self.master = LabelFrame(root, text="Vermiculture", font=(
            'arial', 14, 'bold'), width=1000, bg='white')
        self.master.pack(side=BOTTOM)

        # input_var is a frame inside the master located at the left to organize the inputs
        self.input_var = LabelFrame(
            self.master, text="Input", font=('arial', 12), bg='white')
        self.input_var.pack(side=LEFT)

        self.temp_label = Message(
            self.input_var, text="--Temperature--", width=600, bg='white')
        self.temp_label.pack()
        self.temp_field = Entry(self.input_var, width=7, bg='white')
        self.temp_field.pack()

        self.humi_label = Message(
            self.input_var, text="--Humidity--", width=600, bg='white')
        ...
        self.output_var = LabelFrame(
            self.master, text="Output", font=('arial', 12), bg='white')
        self.output_var.pack(side=LEFT)

        self.fan_label = Message(
            self.output_var, text="--Fan--", width=600, bg='white')
        self.fan_label.pack(side=TOP)
        self.fan_field = Entry(self.output_var, width=15, bg='white')
        self.fan_field.pack(side=TOP)

        self.heat_label = Message(
            self.output_var, text="--Heaters--", width=600, bg='white')
        self.heat_label.pack(side=TOP)
        self.heat_field = Entry(self.output_var, width=15, bg='white')
        self.heat_field.pack(side=TOP)

        self.pump_label = Message(
            self.output_var, text="--Pump--", width=600, bg='white')
